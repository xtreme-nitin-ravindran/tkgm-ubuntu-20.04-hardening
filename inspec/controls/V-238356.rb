  ...
  if is_system_networked
    ...
    if chrony_conf_exists
      describe "time sources" do
        server_entries = command('grep "^server" /etc/chrony/chrony.conf' || true).stdout.strip.split("\n").entries
        if server_entries.length() == 0
          describe "server entries in /etc/chrony/chrony.conf" do
            subject { server_entries }
            it { should be > 0 }
          end
        else
